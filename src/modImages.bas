...
Public Const ImagesCablePath As String = "\\naa.fujikurausa.com\dfs\SPB\Apps\prod\PREMISE\Premise Cable Images\"
Enum ImageTypes
Isometric = 2
CrossSection = 1
End Enum
...
SaveImageFile = False
Select Case Images!ImageTypeID
    Case Isometric
        fileDestination = ImageIsometricPath
    Case CrossSection
        fileDestination = ImageCrossSectionPath
End Select

'destination = fileDestination & Images!ImageName
...
qry = "SELECT * FROM cutsheet.ImageGroupReference"
rst.Open qry, cnn, adOpenKeyset, adLockOptimistic
    With rst
        .AddNew
